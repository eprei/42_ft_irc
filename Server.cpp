...
Server::Server(){
	this->_nOfClients = 10; // TO DELETE; just to test
}
...
// SET THE GROUPS OF FD THAT WILL BE CHECKED BY SELECT
	...
	FD_SET(this->_serverSocket, &current_sock);
		...
		for (int i = 0; i < FD_SETSIZE; i++) // TO OPTIMIZE: change FD_SETSIZE by the number or users to reduce the amount of iterations
		{
			if (FD_ISSET(i, &ready_sock)){
				...
				if (i == this->_serverSocket){
					...
					std::cout << "serversocket = " << this->_serverSocket << "\tclientSocketLocal = " << clientSocketLocal << std::endl;
